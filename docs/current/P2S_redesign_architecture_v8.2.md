...
> ├────────────────────────────────────────────────────────────┤
> │ P2S_redesign_architecture_v8.2.md（本文件）                   │
> │   要做什麼、長什麼樣、怎麼運作、技術選型                     │
...
| RQ3：理解、記憶與傳播效果 | 測試 P2S 內容是否提升理解與傳播 | MVP 2A-2C + MVP 5 | Presentation Planning / TTS / visual / final video + controlled study / online deployment |
...
class SceneDraft(BaseModel):
    scene_id: str                  # MVP2A 起改為 str（例如 "scene_001"），與 PresentationScene 互相引用
    purpose: Literal[
        "hook", "problem", "method", "result",
        "limitation", "takeaway", "transition"  # "transition" 於 MVP2A 新增
    ]
    claim_ids: list[str]           # transition 場景可為空列表

    voice_text: str
    subtitle_text: str
    target_duration_sec: float

    # ── Presentation 欄位（MVP2A 新增）──────────────────────────────
    presenter_mode: Literal[
        "speaking_on_camera",      # 角色正面主講（預設）
        "speaking_with_overlay",   # 角色主講 + 素材疊加
        "silent_presence",         # 角色在場但不說話
        "minimized",               # 角色縮小至角落
        "off_screen",              # 角色不出現
    ] = "speaking_on_camera"

    visual_focus: Literal[
        "presenter",               # 畫面焦點在角色（預設）
        "supporting_asset",        # 角色在場，素材為輔
        "split",                   # 分割畫面
        "asset_fullscreen",        # 素材全螢幕
        "text_card",               # 文字卡片
    ] = "presenter"

    asset_policy: Literal[
        "none",                    # 此幕不需要素材
        "optional",                # 有素材更好，沒有也可（預設）
        "required",                # 此幕必須有素材
    ] = "optional"

    asset_intent: str | None = None          # 素材用途說明，例如 "圖示模型架構"
    asset_type_hint: Literal[
        "none", "paper_figure", "diagram",
        "metaphor_image", "chart",
    ] = "none"

    background_mode: Literal[
        "static_clean",            # 低干擾純色背景（預設）
        "static_thematic",         # 主題靜態背景
        "simple_gradient",         # 漸層背景
        "custom",                  # 自訂
    ] = "static_clean"

    notes_for_render: str | None = None      # 給渲染器的備注（不影響文案審查）

    # ── Deprecated 欄位 ──────────────────────────────────────────
    visual_intent: str | None = None         # ← deprecated，改由 asset_intent 承接
    visual_type: str | None = None           # ← deprecated（MVP2A 起），由 visual_focus +
                                             #   asset_policy + asset_type_hint 取代
...
p2s/
  ...
  p2s_core/
    ...
    models/
      ...
      presentation.py             # PresentationProfile, PresentationPlan, PresentationScene（MVP2A 新增）
      storyboard.py
      asset.py
    ...
    services/
      ...
      claim_extraction.py
      narrative_planning.py       # MVP2A：從 claims 產生敘事大綱
      presentation_planning.py    # MVP2A：從 claims + 敘事大綱產生 SceneDraft + PresentationPlan
      script_generation.py        # deprecated（MVP2A 起由上兩個 service 取代）
      visual_planning.py
      tts_service.py
      media_service.py
      frame_processor.py
    ...
    presentation_profiles/          # MVP2A 新增
    ...
    reviewers/
      paper_fidelity.py
      claim_evidence.py
      script_grounding.py         # MVP2A：voice_text 可追到 claim/evidence
      style_rule.py               # MVP2A：forbidden phrases / 句子長度 / 字幕長度
      presentation_structure.py   # MVP2A：presenter visibility / asset density 審查
      visual_alignment.py
      tts_quality.py
  ...
  runs/
    {project_id}/
      ...
      figures/
      claims.json
      narrative_plan.json          # MVP2A 新增
      scenes.json
      presentation_plan.json       # MVP2A 新增
      asset_plan.json              # MVP2B 新增（取代 storyboard.json）
      storyboard.json              # deprecated（MVP2B 起由 asset_plan.json 取代）
      assets/
      audio/
      frames/
      segments/
  ...
  docs/                              # 所有文件統一管理
    current/                         # 當前有效文件，開工依據
      P2S_redesign_architecture_v8.2.md
      p2s_research_direction_v5.md
      ISSUES.md
    sprints/                         # 各 sprint 的歷史快照，唯讀
      MVP0/
        IMPLEMENTATION_PLAN_MVP0_v3.md
        MVP0_STATUS.md
      MVP1/
        IMPLEMENTATION_PLAN_MVP1_v2.md
        MVP1_STATUS.md
      # MVP2A/, MVP2B/, ... 依序新增
    archive/                         # 被取代的設計文件，供對照用，不作為實作依據
      architecture/
        v7/                          # v7 退役時存入
          P2S_redesign_architecture_v7.md
          MIGRATION_v7_to_v8.md      # 記錄升版時機、變更內容、衝突與處理方式
        # v8/, v9/, ... 未來升版時依序新增
```
...
```text
docs/current/    → 永遠只放當前有效版本。升版時，舊版移入 archive/，不在 current/ 保留舊檔。
docs/sprints/    → sprint 結束後存入，之後不再修改。代表「當時做了什麼決定」的歷史記錄。
docs/archive/    → 被取代的 Architecture Spec 與對應的 migration log。
                   每個 vN/ 子資料夾對應一個退役版本，內容為：
                     - 退役版本原檔（不修改）
                     - MIGRATION_vN_to_vM.md（升版記錄）
```

**Migration log 格式**（每次 Architecture Spec 升版時建立）：

```text
MIGRATION_vN_to_vM.md 應包含：
  1. 升版時機與原因
  2. 變更內容摘要（新增 / breaking / deprecated）
  3. 與已實作程式碼的衝突清單（影響範圍 + 處理方式 + 處理時機）
  4. 衝突處理狀態表（⏳ 待處理 / ✅ 已修）
  5. 共存期間規則
  6. 歸檔計畫（何時結束共存、移入哪個路徑）
```
...
script_generation.py
  ...
  → 包含 voice_text / subtitle_text / asset_intent
...
narrative_plan.json               # narrative_planning 輸出
scenes.json                       # presentation_planning 輸出（SceneDraft list）
presentation_plan.json            # presentation_planning 輸出（PresentationPlan）
asset_plan.json                   # asset_preparation 輸出（取代舊 storyboard.json）
...
`SceneDraft`（1.2 節）是 presentation planning 階段的**草稿輸出**，由 LLM 生成後存入 project_state 待審查。通過 script + presentation reviewer gate 後，才轉換為正式的 `Scene`，並補上 `audio_path`、`asset_paths`、`locked_fields`、`review_status` 等執行期欄位。

```text
NarrativePlanningService → narrative_plan.json
PresentationPlanningService → SceneDraft list（含 presenter/asset/background 規劃）
→ ScriptGroundingReviewer + StyleRuleReviewer + PresentationStructureReviewer 審查通過
→ 轉換為 Scene（正式）
...
class Scene(BaseModel):
    scene_id: str                  # MVP2A 起為 str，與 SceneDraft 一致
    purpose: Literal[
        "hook", "problem", "method", "result",
    ...
    # ── Presentation 欄位（從 SceneDraft 繼承，MVP2A 新增）─────────
    presenter_mode: Literal[
        "speaking_on_camera",
    ...
    asset_policy: Literal["none", "optional", "required"] = "optional"
    ...
    # ── 執行期欄位（AssetService / ReviewCommittee 填入）──────────
    visual_prompt: str | None = None
    selected_figure_ids: list[str] = []
    character_expression: str | None = None  # VRM 表情提示（MVP2C+）
    character_motion: str | None = None      # VRM 動作提示（MVP2C+）
    ...
    visual_type: str | None = None          # ← deprecated（MVP2A 起），由 visual_focus +
                                            #   asset_policy + asset_type_hint 取代
    character_presence: str | None = None   # ← deprecated，改用 presenter_mode
```

### 5.3b Presentation schema（MVP2A 新增）

```python
class PresentationProfile(BaseModel):
    """可替換的影片呈現規則，控制 presenter / asset / background 策略。"""
    profile_id: str
    name: str
    description: str
    ...
    presenter_min_visibility_ratio: float = 0.6   # 角色可見幕數 / 總幕數
    fullscreen_asset_allowed: bool = True
    fullscreen_asset_max_ratio: float = 0.25      # 全螢幕素材幕數 / 總幕數上限
    allow_repeated_background: bool = True
```

```python
class PresentationScene(BaseModel):
    """PresentationPlan 中的每幕呈現規劃，與 SceneDraft.scene_id 對應。"""
    scene_id: str
    scene_type: Literal[
        "presenter_only",
    ...
    presenter_mode: Literal[
        "speaking_on_camera", "speaking_with_overlay",
        "silent_presence", "minimized", "off_screen",
    ]
    visual_focus: Literal[
        "presenter", "supporting_asset", "split",
        "asset_fullscreen", "text_card",
    ]
...
class PresentationPlan(BaseModel):
    """整份影片的呈現規劃，落盤為 presentation_plan.json。"""
    project_id: str
    profile_id: str
    scenes: list[PresentationScene]
    presenter_visibility_ratio: float    # 實際計算值，供 PresentationStructureReviewer 檢查
...
> **設計原則**：`PresentationProfile` 是可替換的，`presenter_first_default` 只是第一個預設。
> 未來可新增 `slide_first_academic`、`no_presenter_social_short`、`figure_first_result_explainer` 等 profile。
> 架構上不應把 Scene 寫死成「一定有 3D 角色」或「一定有素材」。
...
### 5.5 Code Version Metadata（v8.1 補丁，v8.2 延續）

P2S 不應把 `runs/` 目錄上傳到 GitHub，但每次生成出的 artifact 仍必須能追溯到「當時是哪一版程式碼產生的」。因此 `project_state.json` 與每個 stage 都應記錄輕量級 code version metadata。

這個設計的目的不是同步 `runs/` 與 Git，而是建立以下對應：

```text
Git commit / branch / dirty state
→ stage execution
→ project_state.json
→ generated artifacts such as claims.json / scenes.json / presentation_plan.json
```

#### 5.5.1 CodeVersion schema
...
```text
commit      = 當時的 git commit hash。
branch      = 當時所在 branch。
dirty       = 工作區是否有未提交修改。
captured_at = 取得 code version 的時間。
source      = "git" 表示成功從 git 取得；"unknown" 表示環境不是 git repo 或 git unavailable。
```

#### 5.5.2 ProjectState / StageState 更新
...
頂層 `code_version` 代表最近一次保存 project_state 時觀測到的程式版本；stage 內的 `code_version` 代表該 stage 實際執行時的程式版本。若某個 project 跨越多個 commit 執行多個 stage，應以 stage-level `code_version` 為準。

#### 5.5.3 Git metadata capture 規則
...
```text
1. stage runner 開始執行 stage 前，讀取目前 git commit / branch / dirty。
2. 寫入 stages.<stage_name>.code_version。
3. 每次 save_state 時，可更新 ProjectState.code_version 為當下觀測值。
4. 若不是 git repo、git command 不存在、或 subprocess 失敗，不得讓 pipeline crash；回傳 source="unknown"。
5. dirty=true 時不可視為錯誤，只表示本次 artifact 不是由 clean commit 單獨產生。
```

#### 5.5.4 Git 與 runs/ 的邊界
...
- docs
- config template
- persona/style/presentation profile template

runs/ 不應保存到 GitHub：
- PDF 原文
- extracted_text.md
- claims.json
- scenes.json / scenes_rewritten.json
- review outputs
- audio / image / video artifacts
- API results 或可能含敏感資訊的中間輸出
```

因此，code version metadata 是「artifact provenance」，不是「把 artifact 納入 version control」。

#### 5.5.5 測試要求

```text
- 在 git repo 中，capture_code_version() 能取得非空 commit。
- dirty state 能被記錄為 true / false。
- 非 git 環境或 git command 失敗時，source="unknown"，pipeline 不 crash。
- 執行任一 stage 後，stages.<stage>.code_version 有值。
- 舊 project_state migration 後若缺少 code_version，不應造成 status / run 指令失敗。
```
...
→ `p2s run --from-stage presentation_planning`：從指定 stage 開始往後跑
...
Phase 4  presentation_planning  （原 script_generation）
Phase 5  asset_preparation      （原 storyboard_planning）
...
- **負責 service**：`narrative_planning.py`
...
### Phase 4：Presentation Planning（原 script generation）

- **Stage 名稱**：`presentation_planning`
- **負責 service**：`narrative_planning.py`、`presentation_planning.py`（原 `script_generation.py` 拆分）
- **負責 reviewer**：`script_grounding.py`、`style_rule.py`、`presentation_structure.py`、`arbiter.py`
- **輸入**：`claims.json` + persona + style + `presentation_profile` + 敘事大綱
- **輸出**：`scenes.json`（SceneDraft list，含 presenter / asset / background 規劃）+ `presentation_plan.json`

生成規則：

- 每句 voice_text 必須對應 claim_ids（transition 場景除外）。
- subtitle_text 比 voice_text 更短。
- 每幕只承載一個主要概念。
- 使用短影音節奏，但禁止犧牲嚴謹性。
- 遵守 Presentation Profile 規則（asset density / presenter visibility ratio）。
- 不強制每幕都有素材圖片；asset_policy = "none" 或 "optional" 均合法。

Reviewer：

- **ScriptGroundingReviewer**：voice_text 是否都能追到 claim_id / evidence，不得新增 unsupported claim
- **StyleRuleReviewer**：文字是否符合 selected Style（forbidden phrases、句子長度、字幕長度）
- **PresentationStructureReviewer**：presenter visibility ratio / asset density / fullscreen asset ratio 是否符合 Presentation Profile；是否每幕都強制塞圖；背景策略是否一致
- **Arbiter**（彙整 → GateDecision）

> ⚠ **Deferred（MVP3）**：`devil_advocate.py`、`script_quality.py`、`persona_consistency.py` 不在 MVP2A 範圍內。

### Phase 5：Asset & Render Preparation（原 Storyboard & visual planning）

- **Stage 名稱**：`asset_preparation`
- **負責 service**：`visual_planning.py`
- **負責 reviewer**：`visual_alignment.py`
- **輸入**：`presentation_plan.json`（已通過 presentation gate）+ `figures/`
- **輸出**：`asset_plan.json`（每個 scene 對應的 TTS plan + visual asset plan + figure 選擇）

> **注意**：視覺焦點（`visual_focus`）、素材策略（`asset_policy`）、背景模式（`background_mode`）已在 Phase 4 Presentation Planning 決定並寫入 `SceneDraft`。Phase 5 的責任是依照這些決策**準備具體素材**，而非重新決定畫面策略。

目標：決定每幕的具體素材來源與 TTS 參數。

```text
asset_type_hint = "paper_figure"   → 從 figures/ 選取最相關圖表
asset_type_hint = "diagram"        → 生成 diagram prompt
asset_type_hint = "metaphor_image" → 生成 image prompt
asset_policy    = "none"           → 跳過素材，只做 TTS
```
...
> ⚠ **Deprecated**：`storyboard.json` 在 MVP2B 起由 `asset_plan.json` 取代。`storyboard_quality.py` 改由 `visual_alignment.py` 承接。
...
- **輸入**：通過 visual alignment gate 的 `asset_plan.json`
...
Priority 3: Simple HTML diagram（用 HTML+CSS 畫的簡圖，例如箭頭流程）
  ↓ 失敗條件：scene 的 `asset_intent` 無法用標準 HTML diagram template 表達
...
async def generate_scene_visual(scene: Scene) -> AssetResult:
    fallback_chain = [
        ...
        ("html_diagram", lambda: media_service.render_html_diagram(scene.asset_intent)),
...
- presenter_mode
- visual_focus
- asset_policy / asset_type_hint / asset_intent
- background_mode
- visual_prompt
- character_expression / character_motion（VRM，MVP2C+）
...
### MVP 2A：Claim-grounded Presentation Planning

> **MVP2A 要做的不是「產生短影音文案」，而是「從 grounded claims 產生可由 3D 角色主講、可插入素材輔助、背景低干擾的 presentation plan」。**

- `PresentationProfile` / `PresentationPlan` / `PresentationScene` schema。
- `presenter_first_default` presentation profile。
- `SceneDraft` 加入 presenter / asset / background / visual_focus 欄位（詳見 1.2 節）。
- `narrative_planning.py`：從 claims.json 產生 narrative_plan.json。
- `presentation_planning.py`：從 claims + narrative_plan 產生 scenes.json + presentation_plan.json。
- 新增 reviewers：`script_grounding.py`、`style_rule.py`、`presentation_structure.py`。
- CLI 支援 `p2s run --stage presentation_planning`。
- 輸出：`narrative_plan.json` / `scenes.json` / `presentation_plan.json` / `reviews/presentation_review_rev001.json`。

### MVP 2B：Asset & Render Preparation

- `visual_planning.py` 強化：依照 presentation_plan 準備具體素材計畫。
- `asset_plan.json`（取代舊 `storyboard.json`）。
- TTS plan 生成（不含真實 TTS 呼叫）。
- figure 選取邏輯。

### MVP 2C：Actual Media Generation / Composition

- TTS generation（`tts_service.py`）。
- visual asset generation（`media_service.py`）。
- HTML template frame rendering（`frame_processor.py`）。
...
## 10A. Engineering Cadence：MVP Slices + Hardening Checkpoints（v8.2 新增）

P2S 使用的「MVP」不是傳統產品語境中「已可交付給外部使用者的最小產品」，而是 **engineering slice**：每個 MVP 用來建立一段 pipeline 的穩定資料合約、schema、CLI stage、落盤 artifact 與基本測試。換句話說，`MVP complete` 的意思是「這個階段的輸出已經足夠讓下一階段穩定讀取」，不是「這個階段的品質已經完成」、「研究上已經足夠」或「可以正式對外使用」。

這個定義對 P2S 很重要，因為 P2S 是長 pipeline，而不是單一功能：
...
→ asset_plan.json
→ audio / assets / frames / segments
→ final video
→ review / UI / evaluation
```

如果每個階段都在沒有穩定 artifact contract 的情況下往下接，錯誤會在後面變得更難追蹤；但如果每個階段都要求一開始就品質完美，專案又會卡死在前處理或單一模組。因此，P2S 採用雙軌節奏：
...
### 10A.1 MVP 與 Hardening 的分工

```text
MVP slice
- 目標：接通一段 pipeline，建立穩定輸入/輸出合約。
- 產物：schema、service、CLI stage、artifact、basic tests、smoke test。
- 驗收語義：contract complete，不代表 quality complete。

Hardening checkpoint
- 目標：回補資料品質、真實案例、邊界條件、校準測試與 reviewer/gate 穩定性。
- 產物：quality report、golden fixtures、real-paper smoke set、false positive/false negative 檢查、migration 或 compatibility patch。
- 驗收語義：quality improved，降低下游放大錯誤的風險。
```

因此，P2S 不應把「MVP 完成」解讀成「這部分永遠不用回來看」。相反地，MVP 建立的是可回來 harden 的穩定支架。每次 hardening 的實作細節不寫在 Architecture Spec 主文，而是另外建立 `IMPLEMENTATION_PLAN_*` 文件，和 MVP sprint contract 使用同一種格式管理。

### 10A.2 何時插入 Hardening Checkpoint

Hardening 不應任意插入，而應在「下游錯誤成本即將變高」之前插入。常見觸發條件如下：

```text
1. 某個 artifact contract 已經穩定，但 smoke test 暴露上游資料品質缺口。
2. 下一個 MVP 會把錯誤變成更昂貴的媒體 artifact，例如 TTS、圖片、影片或 UI interaction。
3. reviewer 或 validator 的 false negative 可能導致錯誤一路流到 final output。
4. real-paper smoke 的行為和 synthetic tests 差距明顯。
5. migration / backward compatibility 開始影響舊 project runs。
```

這個原則尤其適用於 media generation 前。只要進入 TTS、image generation、HTML rendering、ffmpeg composition，錯誤修正成本就會顯著上升；因此在 full media generation 之前，必須檢查上游 extraction、evidence、figure metadata 與 asset plan 是否足夠穩定。

### 10A.3 建議節奏表

以下節奏是治理原則，不是固定不可變排程。實際細節仍由各 `IMPLEMENTATION_PLAN_*` 文件決定。

| Pipeline range | MVP goal | Suggested hardening checkpoint | Hardening focus | Status / timing note |
|---|---|---|---|---|
| MVP0–MVP1 | state / CLI / extraction / claims | HARDEN-1 | PDF extraction、section detection、evidence matching、figure metadata、real-paper calibration | **Current next checkpoint**；因目前已完成到 MVP2B，HARDEN-1 應在 MVP2B 後、MVP2C-thin 前補回 |
| MVP2A–MVP2B | presentation plan / asset plan | HARDEN-2 | scene quality、presentation consistency、asset plan edge cases、figure/asset selection quality | **Do not let this disappear.** MVP2A / MVP2A-2 / MVP2B 已完成，部分 asset plan edge cases 已在 MVP2B v4 contract 中處理；HARDEN-2 應在 HARDEN-1 plan 中明確標記為「合併檢查、推遲、或另開 sprint」 |
| MVP2C | minimal media generation / first playable video | HARDEN-3 | TTS timing、subtitle readability、template rendering、segment composition、audio loudness | 在 MVP2C-thin 完成後插入，避免 media artifact 品質債累積 |
| MVP3 | review committee / gates | HARDEN-4 | reviewer golden dataset、false negative calibration、prompt injection cases、arbiter stability | 在 reviewer gate 進入研究評估前插入 |
| MVP4 | inspection UI | HARDEN-5 | human editing workflow、revision history usability、lock/regenerate UX、study logging | 在使用者研究或正式 UI 評測前插入 |

**HARDEN-2 timing note**：HARDEN-2 的建議位置容易被誤讀成「MVP2B 之後再獨立處理」，但目前專案實際上已完成 MVP2A、MVP2A-2 與 MVP2B，且 MVP2B v4 implementation plan 已經處理了多個 asset plan edge cases。因此，下一份 `IMPLEMENTATION_PLAN_HARDEN1_EXTRACTION_EVIDENCE.md` 必須明確寫出 HARDEN-2 的狀態：

```text
Option A: HARDEN-2 部分合併進 HARDEN-1 的 validation/smoke scope
Option B: HARDEN-2 推遲到 MVP2C-thin 後，以 scene/media mismatch 為主要檢查
Option C: 另開 IMPLEMENTATION_PLAN_HARDEN2_SCENE_ASSET_QUALITY.md
```

若沒有做這個決策，未來回頭檢查「scene quality / presentation consistency / asset plan quality 是否被認真 harden 過」時，會找不到明確紀錄。

### 10A.4 目前實際路線（v8.2 決策）

理想上，HARDEN-1 可在 MVP1 後就插入；但本專案實際已經先完成 MVP2A、MVP2A-2 與 MVP2B。MVP2B 已建立 `asset_plan.json` contract，也讓上游缺口更清楚：若 extraction 沒有穩定 figure metadata，後續 visual asset planning 只能依賴 fallback。

因此 v8.2 對當前路線做出以下治理決策：
...
MVP2C-thin 之後不是開放式結尾。完成 thin 版本後，必須插入 HARDEN-3（TTS timing、subtitle readability、segment composition、audio loudness），再決定是否進入 full media generation 或直接推進至 MVP3 review committee。若不明確寫出這段，MVP2C-thin 完成後的「下一步模糊」壓力將重演 HARDEN-1 被跳過的同樣風險。

HARDEN-2（scene quality / presentation consistency / asset plan edge cases）的處置由 HARDEN-1 implementation plan 明確決定；詳見 `MIGRATION_architecture_v8.1_to_v8.2.md` N-004。

`MVP2C-thin` 指的是先建立最小可播放影片閉環，而不是一次完成完整 media generation 系統。它應優先使用穩定、可 fallback 的方式，例如 Edge-TTS、text card、static clean background 與最小 ffmpeg composition。完整圖像生成、VRM rendering、lip sync、複雜模板與高品質影片效果應在後續 hardening 或更高 MVP 中逐步導入。

### 10A.5 Architecture Spec 與 Implementation Plan 的邊界

Architecture Spec 只定義：

```text
- 專案長期架構
- 資料流與 artifact contract
- schema / stage / reviewer 的設計原則
- MVP 與 hardening 的治理節奏
- 哪些能力應該屬於哪一層
```

Architecture Spec 不應承載某個 hardening sprint 的細節。例如 HARDEN-1 會包含 section detection、figure caption extraction、evidence matching calibration、golden fixtures 與 real-paper smoke set，但這些細節應寫在：

```text
IMPLEMENTATION_PLAN_HARDEN1_EXTRACTION_EVIDENCE.md
```

同理，之後若新增 HARDEN-2、HARDEN-3，也應各自建立 implementation plan，而不是把 sprint 任務直接塞進 Architecture Spec。這能避免 Architecture Spec 變成任務清單，也能保持它作為長期架構藍圖的穩定性。

### 10A.6 STATUS 文件標準格式

每個 MVP 和 Hardening checkpoint 完成後，對應的 `*_STATUS.md` 必須依照以下模板撰寫。此模板確保「contract complete」與「quality complete」不被混淆，並讓後續 sprint 能清楚知道哪些品質債尚未處理：
...
此模板的完整說明與首次導入記錄見 `MIGRATION_architecture_v8.1_to_v8.2.md` N-003。
...
```text
PDF → extracted_text.md → claims.json → scenes.json → reviews/presentation_review_rev001.json
```
