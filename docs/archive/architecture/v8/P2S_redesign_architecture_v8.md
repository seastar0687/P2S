...
> 實際開工請參考另一份：
>
> - **`docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md`** ← 兩週內可交付的最小閉環，**這份文件才是 sprint contract**
>
> 三份文件的關係：
>
> ```text
> ┌────────────────────────────────────────────────────────────┐
> │ docs/current/p2s_research_direction_v5.md                  │
> │   為什麼做、研究問題是什麼、要回答什麼、怎麼評估             │
> │   讀者：自己 / 指導教授 / 論文 reviewer                     │
> ├────────────────────────────────────────────────────────────┤
> │ P2S_redesign_architecture_v8.md（本文件）                   │
> │   要做什麼、長什麼樣、怎麼運作、技術選型                     │
> │   讀者：開發者 / agentic AI / 工程協作者                    │
> ├────────────────────────────────────────────────────────────┤
> │ docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md           │
...
**實作順序仍以 Engineering MVP 為準**。也就是說，先完成 `docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md` 所定義的 MVP 0 + MVP 0.5，再往 MVP 1 前進。
...
> **MVP 0 與 MVP 0.5 的完整 sprint 計畫詳見 `docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md`**。
...
- CLI：`p2s run --stage <name>` → 跑指定 stage（Architecture Spec 中可視為 extraction stub；實際 MVP0/0.5 sprint 會依 `docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md` 做 PyMuPDF 純文字 extraction）。
...
- 實作 persona/style compatibility check（rigor_level 與 persona 戲劇化程度的一致性）。第一版只做 deterministic rule check，並在 `docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md` 中加入 `test_compatibility_check.py`。

### MVP 1：PDF → claims → script

- PDF extraction 強化：section detection、figure extraction、quality gate、必要時 OCR fallback。（純文字 extraction 已在 MVP 0/0.5 sprint 完成，詳見 `docs/sprints/MVP0/IMPLEMENTATION_PLAN_MVP0_v3.md`。）
- claims extraction。
- evidence mapping。
